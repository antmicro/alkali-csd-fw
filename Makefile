...
BUILDROOT_BUILD_DIR = $(BUILD_DIR)/buildroot/$(BOARD)
...
BR2_EXTERNAL_OVERLAY_DIR = $(BR2_EXTERNAL_DIR)/common/overlay
BUILDROOT_COMMON_OVERLAY_BUILD_DIR = $(BUILDROOT_BUILD_DIR)/overlay
...
BUILDROOT_DEFCONFIG_NAME = $(BOARD)_defconfig
BUILDROOT_OUTPUTS := \
...
IN_BUILDROOT_ENV = env -i PATH=$(PATH)

$(BUILDROOT_COMMON_OVERLAY_BUILD_DIR):
	mkdir -p $@
...
$(BUILDROOT_TOOLCHAIN_TAR_FILE): | $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)
	$(IN_BUILDROOT_ENV) $(MAKE) $(BUILDROOT_OPTS) $(BUILDROOT_DEFCONFIG_NAME)
	$(IN_BUILDROOT_ENV) $(MAKE) $(BUILDROOT_OPTS) sdk
...
APUAPP_OUTPUTS = $(APUAPP_BUILD_DIR)/libvta-delegate.so $(APUAPP_BUILD_DIR)/libvta-driver.so $(APUAPP_BUILD_DIR)/apu-app $(APUAPP_BUILD_DIR)/vta-delegate-test $(APUAPP_BUILD_DIR)/vta-delegate-test-runner
APUAPP_SOURCES = \
...
$(BUILDROOT_OUTPUTS) &: $(APUAPP_OUTPUTS) $(RPUAPP_ZEPHYR_ELF) | $(BUILDROOT_BUILD_DIR) ## Build Buildroot
	cp -r $(BR2_EXTERNAL_OVERLAY_DIR)/* $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/.
	mkdir -p $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/lib/firmware
	cp $(APUAPP_BUILD_DIR)/*.so $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/lib/.
	cp $(RPUAPP_ZEPHYR_ELF) $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/lib/firmware/zephyr.elf
	mkdir -p $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/bin
	cp $(APUAPP_BUILD_DIR)/apu-app $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/bin/.
	cp $(APUAPP_BUILD_DIR)/vta-delegate-test $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/bin/.
	cp $(APUAPP_BUILD_DIR)/vta-delegate-test-runner $(BUILDROOT_COMMON_OVERLAY_BUILD_DIR)/bin/.
	$(IN_BUILDROOT_ENV) $(MAKE) -C $(BUILDROOT_BUILD_DIR) $(BUILDROOT_OPTS) $(BUILDROOT_DEFCONFIG_NAME)
	$(IN_BUILDROOT_ENV) $(MAKE) -C $(BUILDROOT_BUILD_DIR) $(BUILDROOT_OPTS) -j`nproc`
