...
class CommandQueue {
 public:
  ...
  void PushGEMMOp(void** uop_handle, std::function<int(void*)> finit, void* signature, int nbytes) {
  ...
  void PushALUUop(void** uop_handle, std::function<int(void*)> finit, void* signature, int nbytes) {
    UopKernelMap** uptr = reinterpret_cast<UopKernelMap**>(uop_handle);
    if (uptr[0] == nullptr) {
      uptr[0] = new UopKernelMap();
    }
...
int VTAPushGEMMOp(void** uop_handle, std::function<int(void*)> finit, void* signature, int nbytes) {
  vta::CommandQueue::ThreadLocal()->PushGEMMOp(uop_handle, finit, signature, nbytes);
  return 0;
}

int VTAPushALUOp(void** uop_handle, std::function<int(void*)> finit, void* signature, int nbytes) {
